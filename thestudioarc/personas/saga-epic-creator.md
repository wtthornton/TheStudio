...
- **Story sizing for autonomous execution (Ralph):** Stories must be right-sized for autonomous AI agent loops (Ralph). Each story should be completable in a single 15-minute AI loop: create/modify 2-3 files, read 5-6 files for context, run tests once. Too big = the agent times out or loses focus. Too small = wasted loop overhead. The Goldilocks rule: one meaningful unit of work with a clear "done" signal. If a story requires editing 5+ files or understanding 10+ files, split it. If a story is just "create a folder" or "add an import," merge it into the next story.
