...
- **Story sizing for autonomous execution (Ralph):** When planning sprints, verify stories are right-sized for Ralph (autonomous AI agent loops). Each story = one 15-minute AI loop: 2-3 files created/modified, 5-6 files read for context, 1 test run. Stories sized as M (Medium) should be audited — if they touch 5+ files or require understanding 10+ files, split them. Add a "Ralph Loops" column to sprint tables. Target: 1 loop per story for S-sized, 1-2 for M-sized. Any story estimated at 3+ loops should be split.
