"""Epic 64.4 — Expert Performance: Interactive Elements.

Validates that /admin/ui/experts interactive behaviours work correctly:

  §9.14  — Row click opens the sliding inspector/detail panel
  HTMX   — Detail content is loaded via HTMX swap (hx-get, hx-target)
  Filter — Filter/sort controls respond to user input
...
class TestExpertsFilterControls:
    """Filter and sort controls on the experts page must be present and responsive."""

    def test_primary_action_buttons_present(self, page, base_url: str) -> None:
        """At least one action button or control is present on the experts page."""
        _go(page, base_url)
        ...
        if count == 0:
            pytest.skip(
                "No buttons found on experts page — acceptable for read-only view"
            )
    ...
    def test_filter_or_sort_control_exists(self, page, base_url: str) -> None:
        """A filter or sort control exists on the experts page for refining the list."""
        _go(page, base_url)
        ...
        filter_selectors = [
            ...
            "[role='combobox'][aria-label*='tier' i]",
            ...
            try:
                if page.locator(sel).count() > 0:
                    ...
                    assert ctrl.is_visible(), (
                        f"Filter/sort control ({sel!r}) must be visible"
                    )
                    return
            except Exception:  # noqa: BLE001
                continue

        # If no explicit filter control, sortable column headers are acceptable
        headers = page.locator("th")
        if headers.count() > 0:
            pytest.skip(
                "No explicit filter/sort controls found — table column headers "
    ...
    def test_no_permanently_disabled_primary_buttons(self, page, base_url: str) -> None:
        ...
        if count == 0:
            pytest.skip("No explicitly-marked primary buttons found on experts page")
    ...
    def test_sortable_column_headers_are_interactive(self, page, base_url: str) -> None:
        """Column headers with aria-sort or sort buttons must be visible and enabled."""
        _go(page, base_url)

        sortable_selectors = [
            "th[aria-sort]",
            "th[role='columnheader'][tabindex]",
            "th button",
            "th a",
            "th[data-sort]",
        ]
        for sel in sortable_selectors:
            try:
                if page.locator(sel).count() > 0:
                    header = page.locator(sel).first
                    assert header.is_visible(), (
                        f"Sortable column header ({sel!r}) must be visible"
                    )
                    return
            except Exception:  # noqa: BLE001
                continue

        pytest.skip("No sortable column headers found on experts page")
...
class TestExpertsHtmxSwaps:
    """Experts page HTMX controls must carry correct hx-* attributes."""
    ...
    def test_htmx_targets_exist_in_dom(self, page, base_url: str) -> None:
        """hx-target selectors on experts page reference elements in the current DOM."""
        _go(page, base_url)
        ...
        if count == 0:
            pytest.skip("No elements with hx-target found on experts page")
    ...
    def test_expert_row_has_hx_get_for_detail(self, page, base_url: str) -> None:
        """Expert table rows with HTMX carry hx-get pointing to a detail endpoint."""
        _go(page, base_url)

        if not _has_expert_rows(page):
            pytest.skip(
                "No expert rows in table — skipping HTMX row attribute check"
            )
        ...
        if hx_rows.count() == 0:
            ...
            data_rows = page.locator(
                "table tbody tr[data-detail-trigger], table tbody tr[data-href]"
            )
            if data_rows.count() == 0:
                pytest.skip(
                    "Expert rows use neither hx-get nor data-detail-trigger — "
        ...
        assert hx_get_val, "hx-get attribute on expert row must not be empty"
...
class TestExpertsDetailPanel:
    """Clicking an expert row must open the §9.14 sliding inspector panel."""

    def test_detail_panel_trigger_elements_exist(self, page, base_url: str) -> None:
        """At least one element on the experts page can open a detail panel."""
        _go(page, base_url)
        ...
        if not found:
            ...
            if "no expert" in body or "no data" in body or "empty" in body:
                pytest.skip(
                    "Experts page is empty — no rows to trigger detail panel"
                )
            pytest.skip("No detail-panel trigger elements found on experts page")

    def test_row_click_opens_detail_panel(self, page, base_url: str) -> None:
        """Clicking an expert row reveals a §9.14 detail/inspector panel in the DOM."""
        _go(page, base_url)

        if not _has_expert_rows(page):
            pytest.skip(
                "No expert rows in table — skipping detail-panel click test"
            )
        ...
        if not clicked_row:
            pytest.skip("No visible expert rows found to click")
        ...
        assert panel.count() > 0, (
            "Clicking an expert row must reveal a §9.14 side panel/drawer element — "
            "none found after click"
        )

    def test_detail_panel_loads_expert_content(self, page, base_url: str) -> None:
        """The detail panel opened by a row click contains expert-related content."""
        _go(page, base_url)

        if not _has_expert_rows(page):
            pytest.skip("No expert rows — skipping detail content check")
        ...
        has_expert_content = any(kw in body_text for kw in expert_keywords)
        assert has_expert_content, (
            "Detail panel must contain expert-related content (tier, confidence, drift, etc.) — "
    ...
    def test_detail_panel_can_be_dismissed(self, page, base_url: str) -> None:
        """Once opened, the expert detail panel can be closed via close button or Escape."""
        _go(page, base_url)

        if not _has_expert_rows(page):
            pytest.skip("No expert rows — skipping panel dismiss test")
        ...
        assert still_visible_count == 0, (
            "Expert detail panel must be dismissible — panel still visible after close action"
        )
...
class TestExpertsHtmxDetailLoad:
    """HTMX must load expert detail content into the panel swap target."""
    ...
    def test_htmx_detail_swap_triggers_on_row_click(self, page, base_url: str) -> None:
        ...
        if hx_rows.count() == 0:
            pytest.skip(
                "No HTMX-enabled rows found — skipping swap trigger test"
            )
    ...
    def test_no_js_errors_on_row_click(self, page, base_url: str) -> None:
        ...
        if not _has_expert_rows(page):
            pytest.skip("No expert rows — skipping JS error check")
        ...
        assert not js_errors, (
            f"JS errors occurred during expert row click / HTMX swap: {js_errors}"
        )
