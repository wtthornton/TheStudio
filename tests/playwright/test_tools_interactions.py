"""Epic 65.4 — Tool Hub: Interactive Elements.

Validates that /admin/ui/tools interactive behaviours work correctly:

  §9.14  — Tool item click opens the sliding inspector/detail panel
  HTMX   — Detail content is loaded via HTMX swap (hx-get, hx-target)
  Actions — Approval action buttons (approve/reject) are present and clickable
  Panel   — Detail panel can be dismissed via close button or Escape key

These tests verify *interactive behaviour*, not content or appearance.
Content is covered in test_tools_intent.py (Epic 65.1).
Style compliance is covered in test_tools_style.py (Epic 65.3).
"""

from __future__ import annotations

import pytest

from tests.playwright.conftest import navigate

pytestmark = pytest.mark.playwright

TOOLS_URL = "/admin/ui/tools"


def _go(page: object, base_url: str) -> None:
    """Navigate to the tools page and wait for content to settle."""
    navigate(page, f"{base_url}{TOOLS_URL}")  # type: ignore[arg-type]


def _has_tool_rows(page: object) -> bool:
    """Return True when the tools catalog has at least one data row or card."""
    return (
        page.locator("table tbody tr").count() > 0  # type: ignore[attr-defined]
        or page.locator("[data-tool], [class*='tool-card']").count() > 0
    )


# ---------------------------------------------------------------------------
# Action buttons / approval controls
# ---------------------------------------------------------------------------


class TestToolsActionControls:
    """Approval action buttons and controls on the tools page must be present and responsive."""

    def test_primary_action_buttons_present(self, page, base_url: str) -> None:
        """At least one action button or control is present on the tools page."""
        _go(page, base_url)

        buttons = page.locator("button, [role='button']")
        count = buttons.count()

        if count == 0:
            pytest.skip(
                "No buttons found on tools page — acceptable for read-only view"
            )

        first_btn = buttons.first
        assert first_btn.is_visible(), "First action button must be visible"
        assert first_btn.is_enabled(), "First action button must not be disabled"

    def test_approval_action_buttons_present(self, page, base_url: str) -> None:
        """Approval action buttons (approve/reject) are present on the tools page."""
        _go(page, base_url)

        approval_selectors = [
            "button:has-text('Approve')",
            "button:has-text('Reject')",
            "button[aria-label*='approve' i]",
            "button[aria-label*='reject' i]",
            "[data-action='approve']",
            "[data-action='reject']",
            "button[class*='approve']",
            "button[class*='reject']",
        ]
        for sel in approval_selectors:
            try:
                if page.locator(sel).count() > 0:
                    btn = page.locator(sel).first
                    assert btn.is_visible(), (
                        f"Approval action button ({sel!r}) must be visible"
                    )
                    return
            except Exception:  # noqa: BLE001
                continue

        # If no approval buttons found, it may be because no tools need approval
        body_lower = page.locator("body").inner_text().lower()
        if any(kw in body_lower for kw in ("no tools", "empty", "no pending")):
            pytest.skip(
                "No tools pending approval — approval action buttons not expected"
            )
        pytest.skip(
            "No explicit approval action buttons found on tools page — "
            "detail panel may expose them on selection"
        )

    def test_no_permanently_disabled_primary_buttons(self, page, base_url: str) -> None:
        """Primary action buttons must not be permanently disabled."""
        _go(page, base_url)

        primary_btns = page.locator(
            "button.btn-primary, button[data-primary], button.primary"
        )
        count = primary_btns.count()

        if count == 0:
            pytest.skip("No explicitly-marked primary buttons found on tools page")

        for i in range(count):
            btn = primary_btns.nth(i)
            assert btn.is_enabled(), (
                f"Primary button at index {i} must not be permanently disabled"
            )

    def test_filter_or_search_control_exists(self, page, base_url: str) -> None:
        """A filter or search control exists on the tools page for refining the catalog."""
        _go(page, base_url)

        filter_selectors = [
            "select[name*='status' i]",
            "select[name*='filter' i]",
            "select[aria-label*='status' i]",
            "select[aria-label*='filter' i]",
            "[data-filter]",
            "input[type='search']",
            "input[placeholder*='search' i]",
            "input[placeholder*='filter' i]",
            "button:has-text('Filter')",
            "button[aria-label*='filter' i]",
            "th[aria-sort]",
            "th button",
        ]
        for sel in filter_selectors:
            try:
                if page.locator(sel).count() > 0:
                    ctrl = page.locator(sel).first
                    assert ctrl.is_visible(), (
                        f"Filter/search control ({sel!r}) must be visible"
                    )
                    return
            except Exception:  # noqa: BLE001
                continue

        # Column headers provide implicit navigation
        headers = page.locator("th")
        if headers.count() > 0:
            pytest.skip(
                "No explicit filter/search controls found — table column headers "
                "provide implicit navigation"
            )


# ---------------------------------------------------------------------------
# HTMX swap attributes
# ---------------------------------------------------------------------------


class TestToolsHtmxSwaps:
    """Tools page HTMX controls must carry correct hx-* attributes."""

    def test_htmx_elements_have_target(self, page, base_url: str) -> None:
        """Elements with hx-get or hx-post declare hx-target or hx-swap."""
        _go(page, base_url)

        hx_elements = page.locator("[hx-get], [hx-post]")
        count = hx_elements.count()

        if count == 0:
            pytest.skip(
                "No HTMX hx-get/hx-post elements found — page may not use HTMX"
            )

        for i in range(min(count, 10)):
            el = hx_elements.nth(i)
            hx_target = el.get_attribute("hx-target")
            hx_swap = el.get_attribute("hx-swap")
            assert hx_target is not None or hx_swap is not None, (
                f"HTMX element {i} must declare hx-target or hx-swap"
            )

    def test_htmx_targets_exist_in_dom(self, page, base_url: str) -> None:
        """hx-target selectors on tools page reference elements in the current DOM."""
        _go(page, base_url)

        hx_elements = page.locator("[hx-target]")
        count = hx_elements.count()

        if count == 0:
            pytest.skip("No elements with hx-target found on tools page")

        missing: list[str] = []
        for i in range(min(count, 10)):
            target_sel = hx_elements.nth(i).get_attribute("hx-target") or ""
            if not target_sel or target_sel in (
                "this",
                "closest",
                "next",
                "previous",
                "find",
            ):
                continue
            if any(
                target_sel.startswith(kw) for kw in ("closest ", "next ", "find ")
            ):
                continue
            try:
                if page.locator(target_sel).count() == 0:
                    missing.append(target_sel)
            except Exception:  # noqa: BLE001
                pass

        assert not missing, (
            f"hx-target selector(s) not found in DOM: {missing}"
        )


# ---------------------------------------------------------------------------
# §9.14 — Tool item click opens detail panel
# ---------------------------------------------------------------------------


class TestToolsDetailPanel:
    """Clicking a tool item must open the §9.14 sliding inspector panel."""

    def test_detail_panel_trigger_elements_exist(self, page, base_url: str) -> None:
        """At least one element on the tools page can open a detail panel."""
        _go(page, base_url)

        trigger_selectors = [
            "[data-detail-trigger]",
            "[data-panel-trigger]",
            "tr[hx-get]",
            "table tbody tr",
            "[data-tool]",
            "[class*='tool-card']",
            "a[href*='detail']",
            "button[aria-label*='detail' i]",
            "button[aria-label*='view' i]",
        ]
        found = any(page.locator(sel).count() > 0 for sel in trigger_selectors)

        if not found:
            body = page.locator("body").inner_text().lower()
            if "no tool" in body or "no data" in body or "empty" in body:
                pytest.skip(
                    "Tools page is empty — no items to trigger detail panel"
                )
            pytest.skip("No detail-panel trigger elements found on tools page")

    def test_tool_item_click_opens_detail_panel(self, page, base_url: str) -> None:
        """Clicking a tool item reveals a §9.14 detail/inspector panel in the DOM."""
        _go(page, base_url)

        if not _has_tool_rows(page):
            pytest.skip(
                "No tool items in catalog — skipping detail-panel click test"
            )

        row_selectors = [
            "table tbody tr[hx-get]",
            "table tbody tr[data-detail-trigger]",
            "[data-tool][hx-get]",
            "table tbody tr",
            "[data-tool]",
        ]
        clicked_row = False
        for sel in row_selectors:
            rows = page.locator(sel)
            if rows.count() == 0:
                continue
            first_row = rows.first
            if not first_row.is_visible():
                continue
            first_row.click()
            page.wait_for_timeout(600)
            clicked_row = True
            break

        if not clicked_row:
            pytest.skip("No visible tool items found to click")

        panel_sel = (
            "[role='complementary'], [role='dialog'], "
            ".detail-panel, .inspector-panel, .slide-panel, "
            "[data-panel], [id*='detail'], [id*='panel'], "
            "[class*='detail-panel'], [class*='inspector']"
        )
        panel = page.locator(panel_sel)
        assert panel.count() > 0, (
            "Clicking a tool item must reveal a §9.14 side panel/drawer element — "
            "none found after click"
        )

    def test_detail_panel_loads_tool_content(self, page, base_url: str) -> None:
        """The detail panel opened by a tool click contains tool-related content."""
        _go(page, base_url)

        if not _has_tool_rows(page):
            pytest.skip("No tool items — skipping detail content check")

        row_selectors = [
            "table tbody tr[hx-get]",
            "table tbody tr[data-detail-trigger]",
            "table tbody tr",
            "[data-tool]",
        ]
        clicked_row = False
        for sel in row_selectors:
            rows = page.locator(sel)
            if rows.count() == 0:
                continue
            first_row = rows.first
            if not first_row.is_visible():
                continue
            first_row.click()
            page.wait_for_timeout(600)
            clicked_row = True
            break

        if not clicked_row:
            pytest.skip("No visible tool items found to click")

        body_text = page.locator("body").inner_text().lower()
        tool_keywords = [
            "tool",
            "approval",
            "approved",
            "pending",
            "rejected",
            "profile",
            "capability",
            "status",
            "name",
        ]
        has_tool_content = any(kw in body_text for kw in tool_keywords)
        assert has_tool_content, (
            "Detail panel must contain tool-related content (approval, profile, status) — "
            "none of the expected keywords found after opening panel"
        )

    def test_detail_panel_can_be_dismissed(self, page, base_url: str) -> None:
        """Once opened, the tool detail panel can be closed via close button or Escape."""
        _go(page, base_url)

        if not _has_tool_rows(page):
            pytest.skip("No tool items — skipping panel dismiss test")

        row_selectors = [
            "table tbody tr[hx-get]",
            "table tbody tr[data-detail-trigger]",
            "table tbody tr",
            "[data-tool]",
        ]
        clicked_row = False
        for sel in row_selectors:
            rows = page.locator(sel)
            if rows.count() == 0:
                continue
            first_row = rows.first
            if not first_row.is_visible():
                continue
            first_row.click()
            page.wait_for_timeout(600)
            clicked_row = True
            break

        if not clicked_row:
            pytest.skip("No visible tool items found to click")

        close_btn = page.locator(
            "[aria-label='Close'], [aria-label='Dismiss'], "
            "button[data-close], button.close, .panel-close, [data-panel-close]"
        ).first
        if close_btn.count() and close_btn.is_visible():
            close_btn.click()
        else:
            page.keyboard.press("Escape")

        page.wait_for_timeout(400)

        still_open_sel = (
            "[role='complementary'][aria-hidden='false'], "
            ".detail-panel:not(.hidden):not([hidden]), "
            ".inspector-panel:not(.hidden):not([hidden])"
        )
        still_visible_count = page.locator(still_open_sel).count()
        assert still_visible_count == 0, (
            "Tool detail panel must be dismissible — panel still visible after close action"
        )


# ---------------------------------------------------------------------------
# HTMX detail content loading
# ---------------------------------------------------------------------------


class TestToolsHtmxDetailLoad:
    """HTMX must load tool detail content into the panel swap target."""

    def test_htmx_detail_swap_triggers_on_item_click(self, page, base_url: str) -> None:
        """Clicking an HTMX tool row causes the hx-target to receive new content."""
        _go(page, base_url)

        hx_rows = page.locator("table tbody tr[hx-get], [data-tool][hx-get]")
        if hx_rows.count() == 0:
            pytest.skip(
                "No HTMX-enabled tool items found — skipping swap trigger test"
            )

        first_row = hx_rows.first
        if not first_row.is_visible():
            pytest.skip("HTMX item not visible — data may not be loaded")

        before_html = page.locator("body").inner_html()

        first_row.click()
        page.wait_for_timeout(800)

        after_html = page.locator("body").inner_html()
        assert before_html != after_html, (
            "Clicking an hx-get tool item must trigger an HTMX swap that changes the DOM — "
            "body HTML was identical before and after click"
        )

    def test_no_js_errors_on_item_click(self, page, base_url: str) -> None:
        """Tool item click that triggers HTMX detail load must not raise JS errors."""
        _go(page, base_url)

        if not _has_tool_rows(page):
            pytest.skip("No tool items — skipping JS error check")

        js_errors: list[str] = []
        page.on("pageerror", lambda exc: js_errors.append(str(exc)))

        row_selectors = [
            "table tbody tr[hx-get]",
            "table tbody tr[data-detail-trigger]",
            "table tbody tr",
            "[data-tool]",
        ]
        for sel in row_selectors:
            rows = page.locator(sel)
            if rows.count() == 0:
                continue
            first_row = rows.first
            if not first_row.is_visible():
                continue
            first_row.click()
            page.wait_for_timeout(600)
            break

        assert not js_errors, (
            f"JS errors occurred during tool item click / HTMX swap: {js_errors}"
        )

    def test_detail_panel_not_visible_before_interaction(
        self, page, base_url: str
    ) -> None:
        """The detail panel is hidden or absent before any tool item is clicked."""
        _go(page, base_url)

        visible_panel_sel = (
            ".detail-panel:not(.hidden):not([hidden]):not([aria-hidden='true']), "
            ".inspector-panel:not(.hidden):not([hidden]):not([aria-hidden='true'])"
        )
        visible_count = page.locator(visible_panel_sel).count()
        assert visible_count == 0, (
            f"Detail panel must be hidden on initial page load — "
            f"found {visible_count} visible panel(s) before any interaction (§9.14)"
        )
