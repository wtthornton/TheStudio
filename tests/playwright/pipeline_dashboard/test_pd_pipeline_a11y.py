"""Story 76.2 — Pipeline Dashboard: Accessibility WCAG 2.2 AA.

Validates that /dashboard/?tab=pipeline meets WCAG 2.2 AA accessibility
requirements:
...
class TestPipelineAriaLandmarks:
    """Required ARIA landmark regions must be present on the pipeline tab.
    ...
    def test_header_landmark_present(self, page, base_url: str) -> None:
        """Pipeline tab has a <header> element for the application chrome."""
        _go(page, base_url)

        header_count = page.locator("header, [role='banner']").count()
        assert header_count > 0, (
            "Pipeline tab must have a <header> / banner landmark "
...
class TestPipelineHeadingHierarchy:
    ...
    def test_page_has_at_least_one_heading(self, page, base_url: str) -> None:
        """Pipeline tab has at least one heading element (h1–h3)."""
        _go(page, base_url)

        heading_count = page.locator("h1, h2, h3, h4").count()
        assert heading_count > 0, (
            "Pipeline tab must have at least one heading element "
    ...
    def test_empty_state_heading_accessible(self, page, base_url: str) -> None:
        """Empty pipeline state heading is an h3 with accessible text."""
        _go(page, base_url)

        if page.locator("[data-testid='pipeline-rail']").count() > 0:
            pytest.skip("Pipeline rail active — not in empty state")

        heading = page.locator("[data-testid='empty-state-heading'], [data-testid='empty-pipeline-rail'] h3")
        if heading.count() == 0:
            heading = page.locator("[data-testid='empty-pipeline-rail']").locator("h1, h2, h3")

        if heading.count() == 0:
            pytest.skip("Empty state heading element not found")

        heading_text = heading.first.inner_text().strip()
        assert heading_text, "Empty state heading must have non-empty accessible text"
...
class TestPipelineFocusIndicators:
...
class TestPipelineKeyboardNavigation:
    ...
    def test_interactive_elements_reachable(self, page, base_url: str) -> None:
        """Pipeline tab must expose at least one focusable interactive element."""
        _go(page, base_url)
    ...
    def test_skip_link_or_main_landmark_present(self, page, base_url: str) -> None:
        ...
        assert skip_link > 0 or main_element > 0, (
            "Pipeline tab must have a skip-to-main link or a <main> landmark "
...
class TestPipelineTouchTargets:
...
class TestPipelineColorOnlyIndicators:
    ...
    def test_status_badges_not_color_only(self, page, base_url: str) -> None:
        """Stage status and gate result badges must not rely on colour alone."""
        _go(page, base_url)
    ...
    def test_status_elements_have_text_or_label(self, page, base_url: str) -> None:
        ...
        if not status_elements:
            pytest.skip("No [data-status] or status-class elements found on pipeline tab")
...
class TestPipelineImageAltText:
...
class TestPipelineAxeAudit:
    ...
    def test_no_axe_critical_violations(self, page, base_url: str) -> None:
        """axe-core must find zero critical violations on the pipeline tab."""
        _go(page, base_url)
    ...
    def test_no_axe_serious_violations(self, page, base_url: str) -> None:
        """axe-core must find zero serious violations on the pipeline tab."""
        _go(page, base_url)
    ...
    def test_axe_full_report_summary(self, page, base_url: str) -> None:
        ...
        if violations:
            ...
            report = "\n".join(summary_lines)
            assert not blocking, (
                f"Pipeline tab has {len(blocking)} blocking axe violation(s) "
                f"({len(violations)} total):\n{report}"
            )
        else:
            assert True, "No axe violations — pipeline tab is fully WCAG 2.x AA compliant"
