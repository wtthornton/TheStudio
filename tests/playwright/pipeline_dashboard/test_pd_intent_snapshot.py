"""Story 76.4 — Intent Review Tab: Visual Snapshot Baseline.

Captures baseline screenshots for /dashboard/?tab=intent and registers them
for visual-regression comparison on subsequent runs.

Snapshot strategy
-----------------
- Full-page baseline at the canonical 1280x720 viewport.
- Empty state snapshot: the primary visible state when no task is selected.

Baseline file
~~~~~~~~~~~~~
The canonical empty state snapshot is written to:
    tests/playwright/snapshots/pipeline-dashboard/intent-default.png
...
Related suites
~~~~~~~~~~~~~~
- test_pd_intent_intent.py       — semantic content
- test_pd_intent_api.py          — API endpoints
- test_pd_intent_style.py        — style-guide compliance
- test_pd_intent_interactions.py — interactive elements
- test_pd_intent_a11y.py         — WCAG 2.2 AA
"""
...
class TestIntentFullPageSnapshot:
    """Capture and compare a full-page visual baseline for the intent tab.
    ...
    def test_full_page_baseline(self, page: object, base_url: str) -> None:
        ...
        """
        _go(page, base_url)
        result = compare_snapshot(page, "intent-full-page", page_name=PAGE_NAME)
    ...
    def test_full_page_create_baseline(self, page: object, base_url: str) -> None:
        ...
        """
        _go(page, base_url)
        result = create_baseline(page, "intent-full-page-explicit", page_name=PAGE_NAME)
        assert result.passed, result.summary()
    ...
    def test_no_critical_console_errors_during_snapshot(
        ...
        """Intent tab must not emit console errors while the snapshot is captured.

        Console errors during visual capture indicate JavaScript failures that
        may produce a misleadingly partial screenshot.
        """
        _go(page, base_url)
        compare_snapshot(page, "intent-console-check", page_name=PAGE_NAME)
        ...
        assert not critical_errors, (
            f"Intent tab emitted {len(critical_errors)} critical JS error(s) "
            f"during snapshot capture: {critical_errors[:3]}"
        )


# ---------------------------------------------------------------------------
# Empty state snapshot (canonical — intent-default.png)
# ---------------------------------------------------------------------------


class TestIntentEmptyStateSnapshot:
    """Capture the intent empty state as the canonical visual baseline.

    When no task is selected, the intent tab shows the 'No Task Selected'
    empty state.  This is the primary state in the test environment and is
    captured as ``intent-default.png``.
    """

    def test_intent_default_screenshot_written(self, page: object, base_url: str) -> None:
        """Capture intent-default.png to the canonical snapshot directory.

        This satisfies the explicit file-path requirement in Story 76.4:
        ``tests/playwright/snapshots/pipeline-dashboard/intent-default.png``
        """
        _go(page, base_url)
        result = create_baseline(page, "intent-default", page_name=PAGE_NAME)
        assert result.passed, result.summary()

    def test_empty_state_element_snapshot(self, page: object, base_url: str) -> None:
        """Capture a snapshot of the intent empty state element.

        Targets [data-testid='intent-no-task-state'] for an isolated capture.
        Falls back to a full-page snapshot if the element is not found.
        """
        _go(page, base_url)
            ...
            if locator.count() > 0:
                try:
                    ...
                    dest = capture_element_snapshot(
                        page, selector, "intent-empty-state", page_name=PAGE_NAME
                    )
                    assert dest.exists(), (
                        f"Intent empty state snapshot was not written to disk: {dest}"
                    )
                    section_found = True
                    break
        ...
        if not section_found:
            result = compare_snapshot(
                page, "intent-empty-state-fallback", page_name=PAGE_NAME
            )
            assert result.passed, result.summary()
...
class TestIntentSnapshotRegression:
    ...
    def test_full_page_regression(self, page: object, base_url: str) -> None:
        ...
        _go(page, base_url)
        result = compare_snapshot(
            page, "intent-regression-full-page", page_name=PAGE_NAME
        )
        assert result.passed, result.summary()

    def test_empty_state_regression(self, page: object, base_url: str) -> None:
        """Empty state pixel-diff must not exceed the configured threshold."""
        _go(page, base_url)
        result = compare_snapshot(
            page, "intent-regression-empty-state", page_name=PAGE_NAME
        )
        assert result.passed, result.summary()
