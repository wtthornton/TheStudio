"""Epic 68.3 — Quarantine: Style Guide Compliance.

Validates that /admin/ui/quarantine conforms to the TheStudio style guide
(docs/design/07-THESTUDIO-UI-UX-STYLE-GUIDE.md) sections 4–9:

  §4.1     — CSS design tokens registered on :root
  §4.2     — Focus ring color
  §5.1     — Status badge colors (error/failed = red, held = amber)
...
The quarantine page is an error-triage surface. Operators rely on error status
colors (§5.1) to identify held/blocked events at a glance. The red/amber/gray
semantics are therefore a functional requirement — not cosmetic decoration.

Empty-state styling must also convey the emptiness clearly (no broken layouts,
no phantom rows, legible "nothing quarantined" message).

These tests check *how* the page looks, not *what* it contains.
Semantic content is covered in test_quarantine_intent.py (Epic 68.1).
...
def _navigate_to_quarantine(page: object, base_url: str) -> None:
    """Navigate to the quarantine page and wait for main content."""
...
def _has_quarantine_entries(page: object) -> bool:
    """Return True when quarantine rows or cards are present (not empty-state)."""
    has_rows = (
...
class TestQuarantineDesignTokens:
    ...
    driven by CSS custom properties. Their presence on the quarantine page confirms
    ...
    def test_color_focus_ring_token_registered(self, page: object, base_url: str) -> None:
        ...
        _navigate_to_quarantine(page, base_url)
    ...
    def test_page_background_uses_design_token(self, page: object, base_url: str) -> None:
        """Quarantine page background matches the §4.1 gray-50 or white design token."""
        _navigate_to_quarantine(page, base_url)
        ...
        assert is_gray_50 or is_white, (
            f"Quarantine page body background {bg!r} — expected gray-50 (#f9fafb) or "
...
class TestQuarantineFocusRing:
    ...
    def test_focusable_element_focus_ring(self, page: object, base_url: str) -> None:
        ...
        pytest.skip(
            "No focusable element found on quarantine page — skipping focus ring check"
        )
...
class TestQuarantineErrorStatusColors:
    """Quarantine status badges must use §5.1 error-semantic color tokens.

    The quarantine page is an error-triage surface. §5.1 assigns:
      - error / failed / quarantined  → red palette (#ef4444 / #dc2626)
      - held / pending                → amber/yellow palette (#f59e0b / #d97706)
      - resolved / replayed           → green palette (#22c55e / #16a34a)

    Operators must be able to spot quarantined events at a glance without reading
    every row; correct color semantics are therefore a functional requirement.
    """

    def test_status_badge_colors_present(self, page: object, base_url: str) -> None:
        """Status badges on the quarantine page use §5.1 color tokens."""
        _navigate_to_quarantine(page, base_url)
        ...
        for sel in status_selectors:
            ...
            if count > 0:
                ...
                assert result.passed, (
                    f"Quarantine status badge {sel!r} fails §9.3 recipe: {result.summary()}"
                )
                return
        ...
        pytest.skip(
            "No status badge element found on quarantine page — "
            "skipping §5.1 status color check"
        )

    def test_error_status_badge_uses_red_palette(self, page: object, base_url: str) -> None:
        """Error/failed/quarantined badges must use the §5.1 red error palette.

        §5.1 reserves the red palette exclusively for error/failed states.
        Quarantined events are, by definition, error states — their badges must
        use the red palette so operators immediately identify them.
        """
        _navigate_to_quarantine(page, base_url)
        ...
        error_selectors = [
            ...
            "[data-quarantine-status='error']",
            "[data-quarantine-status='failed']",
            ".status-error",
            ".status-failed",
            ".status-quarantined",
            "[class*='status-error']",
        ...
        for sel in error_selectors:
            ...
            if count > 0:
                ...
                if color:
                    ...
                    assert is_red, (
                        f"Error/quarantined badge {sel!r} uses color {color!r} — "
                        "§5.1 requires red palette for error/failed/quarantined states"
                    )
                    return

        pytest.skip(
            "No error/failed/quarantined status badge found — "
            "skipping §5.1 red-palette enforcement check"
        )

    def test_held_status_badge_not_green(self, page: object, base_url: str) -> None:
        """Held/pending quarantine badges must not use the success (green) color (§5.1).

        §5.1 reserves green for pass/success states. A held/pending quarantine
        badge rendered green would falsely suggest the event resolved successfully.
        """
        _navigate_to_quarantine(page, base_url)
        ...
        held_selectors = [
            "[data-status='held']",
            "[data-status='pending']",
            "[data-quarantine-status='held']",
            "[data-quarantine-status='pending']",
            ".status-held",
            ".status-pending",
            "[class*='status-held']",
            "[class*='status-pending']",
        ]
        for sel in held_selectors:
            count = page.evaluate(  # type: ignore[attr-defined]
                f"document.querySelectorAll({sel!r}).length"
            )
            if count > 0:
                ...
                if color:
                    ...
                    assert not is_green, (
                        f"Held/pending quarantine badge {sel!r} uses green color {color!r} — "
                        "§5.1 reserves green for resolved/success states only"
                    )
                    return

        pytest.skip(
            "No 'held/pending' quarantine status badge found — skipping color check"
        )
...
class TestQuarantineTypography:
    """Quarantine page typography must match the style guide type scale (§6.2).

    The quarantine page carries a page title, optional section headings, body
    text describing failure reasons, and table cells. Consistent heading scale
    and body weight help operators parse dense error lists without visual fatigue.
    """
    ...
    def test_table_cell_typography(self, page: object, base_url: str) -> None:
        ...
        if not _has_table(page):
            pytest.skip(
                "No table found on quarantine page — skipping table cell typography check"
            )
...
class TestQuarantineBadgeRecipe:
    """Badges on the quarantine page must conform to §9.3 badge recipe.

    Quarantine status badges and failure-reason labels are the primary badge type
    on this page. They must use the correct sizing, padding, and font weight as
    specified in §9.3 to maintain visual consistency with the broader admin UI
    badge system.
    """
    ...
    def test_badge_recipe(self, page: object, base_url: str) -> None:
        ...
        for sel in badge_selectors:
            ...
            if count > 0:
                result = validate_badge(page, sel)  # type: ignore[arg-type]
                assert result.passed, (
                    f"Badge {sel!r} on quarantine page fails §9.3 recipe: {result.summary()}"
                )
                return

        pytest.skip(
            "No badge element found on quarantine page — skipping §9.3 badge recipe check"
        )

    def test_dark_theme_does_not_break_layout(self, page: object, base_url: str) -> None:
        """Quarantine page layout remains intact when dark theme is applied."""
        _navigate_to_quarantine(page, base_url)
...
class TestQuarantineTableRecipe:
    """Quarantine events table must conform to §9.2 table recipe.

    The quarantine page typically renders held events in a table with columns
    for event ID, failure reason, source repo, and quarantine timestamp.
    ...
    def test_table_recipe(self, page: object, base_url: str) -> None:
        """Quarantine events table matches §9.2 table recipe (thead background, th scope)."""
        _navigate_to_quarantine(page, base_url)

        if not _has_table(page):
            pytest.skip("No table found on quarantine page — card layout acceptable")

        result = validate_table(page, "table")  # type: ignore[arg-type]
        assert result.passed, (
            f"Quarantine events table fails §9.2 recipe: {result.summary()}"
        )

    def test_table_header_scope_attributes(self, page: object, base_url: str) -> None:
        """<th> elements in the quarantine table have scope='col' attribute (§9.2)."""
        _navigate_to_quarantine(page, base_url)
    ...
    def test_failure_reason_column_present_in_table(self, page: object, base_url: str) -> None:
        """Quarantine table includes a failure reason or error column header (§9.2 / §5.1).

        Operators use the quarantine table to triage errors at a glance. The presence
        of a failure/reason column confirms that error context is surfaced at the list
        level, not buried in a detail panel.
        """
        _navigate_to_quarantine(page, base_url)
        ...
        if not has_reason_col:
            pytest.skip(
                "No failure reason column header found in quarantine table — "
...
class TestQuarantineCardRecipe:
    """Quarantine event cards must conform to §9.1 card recipe if present.

    Some implementations render quarantined events as cards rather than table rows.
    Each card must have the correct background, border color, border-radius (≥ 8px),
    and padding (≥ 16px) per §9.1.
    """

    def test_card_recipe_if_present(self, page: object, base_url: str) -> None:
        """Quarantine event cards match §9.1 card recipe (background, border, radius, padding)."""
        _navigate_to_quarantine(page, base_url)
        ...
        card_selectors = [
            ...
            "[class*='detail-panel']",
            "[data-component='card']",
            "[data-component='quarantine-card']",
        ]
        for sel in card_selectors:
            ...
            if count > 0:
                ...
                assert result.passed, (
                    f"Quarantine card/panel {sel!r} fails §9.1 recipe: {result.summary()}"
                )
                return

        pytest.skip(
            "No card element found on quarantine page — skipping §9.1 card recipe check"
        )
...
class TestQuarantineEmptyStateStyle:
    """Quarantine empty state must be styled correctly when no events are present.

    An empty quarantine queue is a positive signal (no errors). The empty state
    must not show a broken layout, phantom table rows, or unstyled placeholder
    text. It must render with the same card/surface recipe and typography as the
    populated state so the page looks intentional, not broken.
    """

    def test_empty_state_no_broken_table(self, page: object, base_url: str) -> None:
        """When quarantine is empty, no phantom table rows appear (§9.2).
        ...
        if not _has_table(page):
            pytest.skip("No table on quarantine page — empty-state table check not applicable")

        if _has_quarantine_entries(page):
            pytest.skip("Quarantine has entries — empty-state test not applicable")
        ...
        if tbody_row_count > 1:
            # Check whether the rows have meaningful text content
            ...
            assert rows_with_content <= 1, (
                f"Found {tbody_row_count} tbody rows when quarantine is empty; "
    ...
    def test_empty_state_body_background_correct(self, page: object, base_url: str) -> None:
        """Quarantine page body background is correct even when empty (§4.1).

        An empty quarantine state must not break the background token — the page
        ...
        is_white = colors_close(bg, "#ffffff")
        assert is_gray_50 or is_white, (
            f"Quarantine page (empty state) body background {bg!r} — "
